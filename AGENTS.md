...
- 长期方向是减少 Python 核心逻辑、强化 Rust 主路径。除 CLI 编排、配置校验、模型 SDK 接入、报告组装等必要边界外，新增核心逻辑和基础能力优先放到 Rust。
...
- **冗余源码与测试删除 Review Spec**：当前生产契约不再需要的源码、测试、fixture、mock、stub、canary、scan budget、旧 adapter、旧 scanner、fallback、re-export 和只为测试存在的 helper，应物理删除或改写到当前真实链路。
...
- `skills/att-mz-protocol/` 是开发版和发行版翻译流程 Skill 的单一事实来源；修改 Skill 协议时优先修改 canonical 源，再运行 `uv run python scripts/generate_skill_protocol.py --write`。
...
- 不涉及源码、测试、schema、构建流程、发行流程或可执行契约的纯文档、Skill 文案、README 或发布说明改动，原则上不在本机运行全量 `uv run pytest`，只执行与改动直接相关的生成物检查、静态检查或针对性验证；若确需全量运行，必须说明触发原因。
