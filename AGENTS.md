...
- 涉及 Python/Rust 源码、测试、schema、构建流程、发行流程或可执行契约的项目交付前，必须执行 `uv run basedpyright` 和全量 Python 业务测试，保持 0 warning、0 error、0 failed。当前全量 pytest 推荐命令为先设置 `$env:ATT_MZ_RUST_THREADS = "1"`，再执行 `uv run pytest -q -n 6 --durations=30 --durations-min=0.5`；若实测调整 worker 数，交付说明必须写明最终 worker 数、总耗时和最慢测试列表。
