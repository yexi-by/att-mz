...
pub fn scan_write_protocol_count_impl(payload_json: &str) -> Result<String, String> {
    let payload: ProtocolPayload = serde_json::from_str(payload_json)
        .map_err(|error| format!("Rust 写入协议计数输入 JSON 解析失败: {error}"))?;
    let count = run_with_optional_pool(|| {
        payload
            .entries
            .par_iter()
            .filter(|entry| probe_protocol_entry(entry).is_err())
            .count()
    })?;
    serde_json::to_string(&json!({ "write_protocol_count": count }))
        .map_err(|error| format!("Rust 写入协议计数输出 JSON 序列化失败: {error}"))
}
...
pub(crate) fn probe_protocol_entry(entry: &ProtocolEntry) -> Result<(), String> {
    ...
    if entry.mode != "nested" {
        return Err(format!(
            "写入协议模式无效: {}; 文本路径={}",
            entry.mode, entry.item.location_path
        ));
    }
...
pub(crate) fn protocol_translation_text(entry: &ProtocolEntry) -> Result<&str, String> {
    let text = entry
        .item
        .translation_lines
        .first()
        .ok_or_else(|| format!("写入协议缺少译文行: {}", entry.item.location_path))?;
    if text.trim().is_empty() {
        return Err(format!("写入协议译文行为空: {}", entry.item.location_path));
    }
    Ok(text)
}
...
mod tests {
    use super::*;
    use crate::native_core::models::NativeTranslationItem;
    use serde_json::json;

    fn nested_entry(translation_lines: Vec<String>) -> ProtocolEntry {
        ProtocolEntry {
            item: NativeTranslationItem {
                location_path: "plugins.js/0/parameters/name".to_string(),
    ...
    #[test]
    fn protocol_probe_rejects_missing_translation_lines() {
        let entry = nested_entry(Vec::new());

        let error = probe_protocol_entry(&entry).expect_err("缺失译文行必须报错");

        assert!(error.contains("写入协议缺少译文行"));
        assert!(error.contains("plugins.js/0/parameters/name"));
    }

    #[test]
    fn protocol_probe_rejects_blank_translation_line() {
        let entry = nested_entry(vec!["   ".to_string()]);

        let error = probe_protocol_entry(&entry).expect_err("空白译文行必须报错");

        assert!(error.contains("写入协议译文行为空"));
        assert!(error.contains("plugins.js/0/parameters/name"));
    }
}
