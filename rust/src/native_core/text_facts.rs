//! 当前文本事实契约 的 Rust 公共事实模型。

use serde::{Deserialize, Serialize};
use sha2::Digest;

/// Text Fact Contract 的当前事实 schema version。
pub(crate) const CURRENT_TEXT_FACT_CONTRACT_VERSION: i64 = 2;
...
impl TextFact {
    ...
    pub(crate) fn validate(&self) -> Result<(), String> {
        if self.schema_version != CURRENT_TEXT_FACT_CONTRACT_VERSION {
            return Err(format!(
                "当前文本事实 schema_version 不受支持: {}，当前要求 {}",
                self.schema_version, CURRENT_TEXT_FACT_CONTRACT_VERSION
            ));
        }
        if !domains::SUPPORTED.contains(&self.domain.as_str()) {
            return Err(format!("当前文本事实 domain 不受支持: {}", self.domain));
        }
        if self.fact_id.trim().is_empty() {
            return Err("当前文本事实 fact_id 为空".to_string());
        }
        if self.scope_key.trim().is_empty() {
            return Err("当前文本事实 scope_key 为空".to_string());
        }
        if self.location_path.trim().is_empty() {
            return Err("当前文本事实 location_path 为空".to_string());
        }
        if self.source_file.trim().is_empty() {
            return Err("当前文本事实 source_file 为空".to_string());
        }
        if self.domain == domains::MV_VIRTUAL_NAMEBOX && self.role.trim().is_empty() {
            return Err("MV 虚拟名字框 speaker 为空，无法写入 当前文本事实".to_string());
        }
        ...
        if self.fact_id != expected_fact_id {
            return Err(format!(
                "当前文本事实 fact_id 与 schema/domain/location/selector/raw_hash 不一致: {}",
                self.fact_id
            ));
        }
        Ok(())
...
impl TextFactRenderPart {
    ...
    pub(crate) fn validate(&self) -> Result<(), String> {
        if self.fact_id.trim().is_empty() {
            return Err("当前文本事实 render part 缺少 fact_id".to_string());
        }
        if self.part_order < 0 {
            return Err(format!(
                "当前文本事实 render part part_order 不能为负数: {}",
                self.part_order
            ));
        }
        if self.part_kind.trim().is_empty() {
            return Err("当前文本事实 render part 缺少 part_kind".to_string());
        }
        if self.template_key.trim().is_empty() {
            return Err("当前文本事实 render part 缺少 template_key".to_string());
        }
        Ok(())
    }
}
...
impl TextFactDomainPayload {
    ...
    pub(crate) fn validate(&self) -> Result<(), String> {
        if self.fact_id.trim().is_empty() {
            return Err("当前文本事实 domain payload 缺少 fact_id".to_string());
        }
        let value: serde_json::Value = serde_json::from_str(&self.payload_json)
            .map_err(|error| format!("当前文本事实 domain payload_json 不是有效 JSON: {error}"))?;
        if !value.is_object() {
            return Err("当前文本事实 domain payload_json 必须是 JSON 对象".to_string());
        }
        Ok(())
    }
}
...
impl TextFactScope {
    /// 根据三类 scope 输入 hash 创建当前文本事实 scope。
    ...
    pub(crate) fn validate(&self) -> Result<(), String> {
        ...
        if self.source_snapshot_hash.trim().is_empty() {
            return Err("当前文本事实 scope 缺少 source_snapshot_hash".to_string());
        }
        if self.rule_hash.trim().is_empty() {
            return Err("当前文本事实 scope 缺少 rule_hash".to_string());
        }
        if self.text_rules_hash.trim().is_empty() {
            return Err("当前文本事实 scope 缺少 text_rules_hash".to_string());
        }
        if self.created_at.trim().is_empty() {
            return Err("当前文本事实 scope 缺少 created_at".to_string());
        }
        ...
        if self.scope_hash != expected_scope_hash {
            return Err(format!(
                "当前文本事实 scope_hash 与 source/rule/text_rules/schema 不一致: {}",
                self.scope_hash
            ));
        }
        ...
        if self.scope_key != expected_scope_key {
            return Err(format!(
                "当前文本事实 scope_key 与 source/rule/text_rules/schema 不一致: {}",
                self.scope_key
            ));
        }
        Ok(())
...
pub(crate) fn build_fact_id(
    ...
    format!("tf:{raw_prefix}:{}", sha256_text(&identity))
}
...
fn validate_hash(field_name: &str, actual: &str, source_text: &str) -> Result<(), String> {
    ...
    if actual != expected {
        return Err(format!(
            "当前文本事实 {field_name} 与约定输入不一致: expected={expected}, actual={actual}"
        ));
    }
    Ok(())
}
...
mod tests {
    use super::{
        CURRENT_TEXT_FACT_CONTRACT_VERSION, TextFact, TextFactInput, TextFactRenderPart,
        TextFactScope, build_scope_key, domains, sha256_text,
    };
    ...
    fn text_fact_hashes_use_raw_visible_and_translatable_inputs() {
        ...
        assert_eq!(fact.schema_version, CURRENT_TEXT_FACT_CONTRACT_VERSION);
    ...
    fn text_fact_rejects_current_non_translation_domains() {
        ...
        for domain in [
            ...
            .expect_err("当前非翻译事实域不得写入 text_facts");
    ...
    fn scope_key_and_scope_hash_include_schema_and_input_hashes() {
        ...
        assert_eq!(scope.schema_version, CURRENT_TEXT_FACT_CONTRACT_VERSION);
        assert_eq!(
            scope.scope_key,
