...
2. 注册游戏：`.\att-mz.exe --agent-mode add-game --path <游戏目录> --json`，后续使用返回的 `<游戏标题>`。
...
14. 若确认某个日文名单、作品名、品牌名、专有名词确实不应翻译，必须写入 `japanese-residual-rules.json`，先 `validate-japanese-residual-rules --json`，再 `import-japanese-residual-rules --json`，禁止全局关闭日文残留检测。
...
16. 写回完成后，向用户说明这是第一版可试玩汉化结果；请用户运行游戏，重点反馈漏翻、误翻、术语不统一、窗口显示异常、剧情语气不自然、按钮菜单看不懂、插件界面仍是日文、图片文字未处理等问题。
...
术语候选必须由主代理统一风格、去空值、查假名残留和译名冲突，再合并到 `terminology/field-terms.json` 并维护 `terminology/glossary.json`。规则类结果必须运行对应 `validate-* --json`，通过后才运行对应 `import-* --json`。大面积错误时要求重做或改由主代理完成，不能直接导入。
