...
- 术语候选、占位符规则、插件规则、事件指令规则和 Note 标签规则的具体正反样例放在 `references/translation-rule-examples.md`。
- 结构化占位符规则的输入格式、校验流程和冲突边界放在 `references/structured-placeholder-rules.md`。
- MV 虚拟名字框规则的候选判断、JSON schema、样例和反例放在 `references/mv-virtual-namebox-rules.md`。
- 只有在判断控制符是否合法、区分标准引擎协议和插件自定义协议、分析事件指令/Note/插件字段、或排查工具报告是否异常时，才打开该参考文档。
- 进入术语候选、占位符收束、插件规则、事件指令规则或 Note 标签规则步骤时，必须读取 `references/translation-rule-examples.md` 中对应小节；只读取当前步骤需要的小节，不把样例全文复制进 prompt 或交付报告。
- 进入结构化占位符规则编写、审查或修复前，必须读取 `references/structured-placeholder-rules.md`；普通正则占位符规则和结构化占位符规则是并列关系，不能互相替代。
- 进入 MV 游戏第零轮虚拟名字框规则编写、审查或修复前，必须读取 `references/mv-virtual-namebox-rules.md`；MZ 游戏不执行该规则流程。
...
- `<游戏目录>`：目标游戏。CLI 可以注册、读取、写回，并在第一次写进游戏文件前生成完整原始 `data` 备份 `data_origin`。只有用户明确允许字体覆盖时，CLI 才能复制候选字体，替换游戏数据里的字体引用，并在存在 `fonts/gamefont.css` 时备份和更新字体族入口。临时工作区也可以放在这里，但必须集中在一个明确目录里，不能散落到游戏根目录各处。
...
- `--json` 命令的 stdout 只读取最终 JSON；`translate`、`quality-report`、`write-back` 等支持 JSON 输出的长任务会在 stderr 持续输出无 ANSI 文本进度条，显示已完成数量、百分比、已用时间、预计剩余时间和当前状态。
...
5. MV 第零轮：如果 `manifest.json` 的 `engine_kind` 是 `mv`，主代理必须先读取 `references/mv-virtual-namebox-rules.md` 和 `<工作区>/mv-virtual-namebox-candidates.json`，亲自填写 `<工作区>/mv-virtual-namebox-rules.json`，运行 `.\att-mz.exe --agent-mode validate-mv-virtual-namebox-rules --game <游戏标题> --input <工作区>/mv-virtual-namebox-rules.json --json`，逐条审查 `details.newly_matched_candidates` 中的 `text`、`following_lines`、`matching_rules` 和 `matches`；任何不是说话人名字框的样本都必须通过收紧规则排除并重新校验。确认校验无 error 且新增命中样本已经审查后，再运行 `.\att-mz.exe --agent-mode import-mv-virtual-namebox-rules --game <游戏标题> --input <工作区>/mv-virtual-namebox-rules.json --json`；确认空规则时文件为 `{"rules":[]}` 且导入命令加 `--confirm-empty`。导入后必须重新运行 `prepare-agent-workspace`，保证第一轮 `speaker_names` 按虚拟名字框规则生成。MZ 游戏跳过本步骤。
6. 子代理任务处理方式确认：主代理说明接下来会处理术语候选、插件规则、事件指令规则和 Note 标签规则，并询问用户选择“当前会话完成”“外部协作任务包”或“混合处理”。多项候选分析会消耗较多上下文和模型额度；额度有限时建议使用外部协作任务包。用户未明确选择前，不要默认消耗大量子代理额度。
7. 第一轮子代理只处理术语候选：主代理按字段译名类别拆分 `field-terms.json`，把独立候选文件交给多个子代理或整理成外部协作任务包；子代理和任务包返回内容都只能作为候选，不能直接写 `terminology/field-terms.json` 或 `terminology/glossary.json`。主代理必须等待全部结果、逐项审查信达雅和译名统一、亲自修改字段译名表，并把 `/c<角色名>`、`"<角色名>"`、`◆<角色名>ｔ` 等字段形式由 Agent 人工判断后规范化为正文术语表的 `terms`，才执行 `import-terminology --json`。
8. 第二轮子代理再处理插件规则、事件指令规则和 Note 标签规则；如果用户选择外部协作任务包，主代理收回候选答案后二次审查，逐项校验并导入。
9. 主代理基于当前会进入正文翻译的完整文本集合，亲自生成、审查、覆盖扫描、校验并导入普通占位符规则；如存在“协议外壳固定、中间文本可翻译”的混合文本，还必须按结构化占位符规则流程单独校验并导入。
10. `validate-agent-workspace --game <游戏标题> --workspace <工作区> --json`。
11. 小批量翻译：`translate --game <游戏标题> --max-batches 1 --json`。
12. 查看翻译进度报告 `translation-status --json`、统一文本清单 `text-scope --json`、覆盖审计 `audit-coverage --json` 和质量检查报告 `quality-report --json`，记录剩余数量、当前规则命中范围、检查没通过的译文类型和是否出现规则性事故。
13. 稳定后进入第 N 轮续跑：每轮执行 `translate --game <游戏标题> --json`，随后重新查看翻译进度报告和质量检查报告。只要剩余数量明显下降，且没有新的规则性事故，就继续下一轮，直到没有“还没成功保存译文的文本”，或剩余量已经适合由 Agent 手动填写译文表。
14. 只有多轮续跑后剩余量适合手动处理，或连续多轮同类失败不再下降并已向用户说明原因时，才导出手动填写译文表。统一使用 `export-pending-translations`；需要抽样或分批时追加 `--limit N`，不传 `--limit` 时一次导出全部剩余文本；填写中文译文行后，用 `import-manual-translations` 交回项目检查并保存。
15. 若确认某个名单、作品名、品牌名、专有名词确实应保留源语言原文，必须写入 `source-residual-rules.json`。日文和英文游戏都使用通用源文残留命令，先 `validate-source-residual-rules --json`，再 `import-source-residual-rules --json`，禁止全局关闭源文残留检测。
16. `audit-coverage --json` 和 `quality-report --json` 都没有 `error` 错误，所有 `warning` 已逐项确认不会影响本轮写进游戏文件，并且用户允许写回后，执行 `write-back --game <游戏标题> --json`。普通写回不会覆盖字体；只有用户另外明确允许覆盖字体时，才执行 `write-back --game <游戏标题> --confirm-font-overwrite --json`。
17. 写回完成后，向用户说明这是第一版可试玩汉化结果；请用户运行游戏，重点反馈漏翻、误翻、术语不统一、窗口显示异常、剧情语气不自然、按钮菜单看不懂、插件界面仍是源语言文本、图片文字未处理等问题。用户给出反馈原文清单后，必须用 `verify-feedback-text --game <游戏标题> --input <反馈原文清单> --json` 反查真实文件。
18. 收到试玩反馈后进入“试玩反馈迭代流程”；每轮修复都必须重新运行质量检查报告，必要时再次写进游戏文件。
19. 用户确认本轮试玩反馈已经处理完成，或明确表示暂不继续修复后，才清理 `<工作区>`；如果工作区由 `prepare-agent-workspace` 生成，优先用 `cleanup-agent-workspace --workspace <工作区> --json`。
...
- 每轮反馈修复都必须走“反馈清单 -> 补规则或补译文 -> audit-coverage -> quality-report -> 用户确认是否再次写进游戏文件 -> verify-feedback-text”的闭环；禁止直接改数据库或手工改游戏 `data/*.json`。
...
- 二次写回由 CLI 直接替换当前激活文件；`data_origin/` 是完整原始 `data` 备份，不要手工移动 `data/` 或 `data_origin/`。二次写回后仍应要求用户继续试玩确认。
...
- `mv-virtual-namebox-rules.json`：MV 专用虚拟名字框规则，合法空结构为 `{"rules":[]}`；必须由主代理在第零轮亲自填写、校验并导入。
- `placeholder-candidates.json`：初始候选控制符报告，只供主代理参考；最终占位符规则必须在术语表和三类外部规则导入后重新确认。
- `placeholder-rules.json`：初始占位符规则草稿；不能跳过最终审查直接导入。
- `structured-placeholder-rules.json`：结构化占位符规则；默认 `{ "paired_shell_rules": [] }`，只用于保护固定协议外壳并把中间显示文本留给模型翻译。
- `terminology/field-terms.json`：字段译名表，只填写 value；用于精确写回地图显示名、数据库名称、系统类型，以及 MZ 标准 `101.parameters[4]` 名字框等游戏字段；其中 `speaker_names` 在 MV 中是从正文首行识别出的虚拟名字框说话人术语，用于译名统一、正文翻译提示词命中，以及重建对应 `401` 说话人行或名字标签，不写回 `101.parameters[4]`。
...
- `plugin-json-string-leaf-candidates.json`：插件参数 JSON 字符串内部的字符串叶子候选，只供判断路径层级，不代表这些文本一定玩家可见。
...
`prepare-agent-workspace` 会优先把 CLI 已保存到当前游戏状态里的字段译名表、正文术语表、MV 虚拟名字框规则、插件规则、事件指令规则、Note 标签规则、普通占位符规则和结构化占位符规则回填到工作区。新游戏或未保存规则时，工作区仍会给出空对象或候选草稿，供主代理分析填写。MV 游戏必须先完成第零轮并重新准备工作区，再处理第一轮术语候选。占位符候选和草稿只是前置参考，最终规则必须在字段译名表、正文术语表和三类外部规则保存后重新生成或逐条确认。
...
- `--json` 输出里的 `status` 是程序给出的阶段结果：`error` 表示当前阶段检查没通过，不能继续；`warning` 只表示非阻断提醒，不能用来绕过任何前置 `error`；`ok` 才能进入下一阶段。
- 每个阶段都必须明确输入、处理逻辑、输出、校验命令和失败恢复动作；缺一项就先补上下文，不把模糊任务交给子代理。
- 主代理负责分析和填写工作区文件，最终是否可用由本项目 CLI 校验决定。

### 源码排障边界

- 正常游戏翻译任务保持黑盒：不靠 A.T.T MZ 项目源码、数据库表结构或 Python 对象来猜外部规则格式。
- 发行版环境不包含项目源码。如果同一合法工作区文件反复触发无法解释的 CLI 错误、Skill/CLI 契约互相矛盾、命令帮助与文档不一致，先停止翻译并报告工具问题。
- 需要阅读或修改 A.T.T MZ 项目源码时，必须切换到源码仓库和开发版 Skill；发行版任务中仍禁止直接修改数据库、绕过 CLI 校验或手改游戏数据文件。
- 排障后若需要继续翻译，必须回到黑盒流程，用公开 CLI 命令重新校验工作区、规则、译文和写回条件。
...
| 翻译与手动填写译文表 | 当前游戏状态、模型配置、质量检查报告 | 小批量试跑、按第 N 轮续跑继续降低剩余数量；数量下降时继续交给模型，数量不降或剩余量适合手动处理时再导出手动填写译文表 | 没有未保存译文，或只剩已向用户说明且适合手动处理的文本 |
| 写进游戏文件 | 已保存译文、无错误且 warning 已确认的质量检查报告、用户许可、字体覆盖许可 | 执行 `write-back --json`，不直接移动 data 目录；未获得字体覆盖许可时不加 `--confirm-font-overwrite` | 已把译文写进游戏目录，并输出机器可读摘要 |
...
| `doctor --game <游戏标题> --no-check-llm --json` | 已注册游戏标题 | `add-game` 已成功 | 检查游戏绑定、规则导入状态和占位符风险，不请求模型服务 | `status` 不是 `error` | 缺规则时只允许继续准备工作区；翻译和写入前仍必须导入规则或用 `--confirm-empty` 明确确认空结果 |
...
| `export-mv-virtual-namebox-candidates --game <游戏标题> --output <候选文件> --json` | MV 游戏标题、候选输出文件 | 游戏已注册且引擎是 MV | 导出 `101` 后首条非空 `401` 候选，供主代理填写虚拟名字框规则 | 输出文件存在，MZ 调用返回 error | MV 候选为空时可确认空规则；候选非空也必须由主代理判断是否真是名字框 |
| `validate-mv-virtual-namebox-rules --game <游戏标题> --input <规则文件> --json` | MV 虚拟名字框规则 JSON | 已阅读 `references/mv-virtual-namebox-rules.md`；规则文件存在 | 校验正则命名分组、模板字段、同一候选多规则命中、源文重建，并报告 `details.newly_matched_candidates` | `status` 不是 `error`，且主代理已确认新增命中样本均为说话人名字框；空规则只允许 warning | 修 `mv-virtual-namebox-rules.json` 后重跑；MZ 不使用该命令 |
| `import-mv-virtual-namebox-rules --game <游戏标题> --input <规则文件> --json` | 已校验 MV 虚拟名字框规则 | validate 无 error，且 `details.newly_matched_candidates` 已逐条审查；空规则必须加 `--confirm-empty` | 保存为当前 MV 游戏虚拟名字框规则，供 `speaker_names`、正文提取和写回使用 | `status` 为 `ok`，或空规则时只有 warning | 导入后重新运行 `prepare-agent-workspace`，让第一轮术语表使用当前规则 |
...
| `import-placeholder-rules --game <游戏标题> --input <规则文件> --json` | 已校验且覆盖扫描通过的规则文件 | validate 与 scan 都已通过；空规则必须额外确认当前候选为空并加 `--confirm-empty` | 保存为当前游戏普通占位符规则 | `status` 为 `ok` | 回到 validate/scan 修规则，不绕过 CLI |
| `validate-structured-placeholder-rules --game <游戏标题> --input <规则文件> --json` | 结构化占位符规则 JSON | 已阅读 `references/structured-placeholder-rules.md`；规则文件存在 | 校验命名分组、模板、冲突边界和样本文本往返 | `status` 不是 `error` | 修 `<规则文件>` 后重跑校验 |
| `scan-structured-placeholder-candidates --game <游戏标题> --input <规则文件> --json` | 结构化占位符规则 JSON 和当前有效正文集合 | validate 已通过 | 扫描协议外壳候选是否被结构化规则覆盖 | 阅读 `summary.uncovered_count` 并确认风险 | 未覆盖时修 `<规则文件>`，再 validate 和 scan |
| `import-structured-placeholder-rules --game <游戏标题> --input <规则文件> --json` | 已校验的结构化规则文件 | validate 已通过，覆盖风险已确认；空规则必须额外确认当前候选为空并加 `--confirm-empty` | 保存为当前游戏结构化占位符规则 | `status` 为 `ok` | 回到 validate/scan 修规则，不绕过 CLI |
...
| `validate-plugin-rules --game <游戏标题> --input <规则文件> --json` | 插件规则 JSON | `plugins.json` 已分析 | 按插件下标定位，校验插件名并检查 JSONPath 命中字符串叶子，同时计算插件哈希供导入后匹配当前配置 | `status` 为 `ok`；空规则文件只能作为候选为空的准备结果，导入时仍必须使用 `--confirm-empty` | 修 `plugin-rules.json`，不读源码猜路径 |
| `import-plugin-rules --game <游戏标题> --input <规则文件> --json` | 已校验插件规则 | validate 已通过 | 导入插件可翻译字段规则 | `status` 为 `ok`，或只有 `deleted_translations_backed_up` warning 已记录备份 | 回到 validate 修规则；若导错，先重新导入正确规则，再用备份文件恢复译文 |
...
| `import-event-command-rules --game <游戏标题> --input <规则文件> --json` | 已校验事件指令规则 | validate 无 error | 导入事件指令文本规则 | `status` 为 `ok`，或只有 `deleted_translations_backed_up` warning 已记录备份 | 回到 validate 修规则；若导错，先重新导入正确规则，再用备份文件恢复译文 |
...
| `import-note-tag-rules --game <游戏标题> --input <规则文件> --json` | 已校验 Note 标签规则 | validate 无 error；空规则必须额外确认当前候选为空并加 `--confirm-empty` | 导入标准 `data/*.json` 的 `note` 标签文本规则 | `status` 为 `ok`，或只有 `deleted_translations_backed_up` warning 已记录备份 | 回到 validate 修规则；若导错，先重新导入正确规则，再用备份文件恢复译文 |
...
| `translate --game <游戏标题> --max-batches 1 --json` | 游戏标题、模型配置 | 工作区已校验并导入 | 小批量试跑正文翻译 | 命令返回 0，质量报告没有新增 `error` | 看 status 和 quality-report，不盲目全量 |
| `translate --game <游戏标题> --json` | 游戏标题、模型配置 | 小批量稳定，或上一轮剩余数量仍在下降 | 继续翻译还没成功保存译文的文本 | 命令返回 0 | 每轮后看翻译进度报告和质量检查报告；数量明显下降就继续第 N 轮，连续多轮不下降才停下报告并决定换模型、改规则或手动填写译文表 |
| `translation-status --game <游戏标题> --json` | 游戏标题 | 至少跑过翻译或导入 | 判断当前还有多少文本没成功保存译文、已成功多少、模型接口是否失败；`pending_count` 表示当前没成功保存译文的文本数，`run_pending_count` 表示最近一次 translate 开始时要处理的文本数 | 数量能解释 | 数量仍明显下降时继续 `translate`；剩余数量已经适合手动处理时再导出手动填写译文表；大量同类失败且多轮不下降时先修规则、换模型或报告无法继续 |
| `text-scope --game <游戏标题> --json` | 游戏标题 | 已注册游戏和当前规则 | 输出统一文本清单，列出每条文本的内部位置、来源类型、规则来源、原文行、是否进入翻译、是否可保存译文、是否可写进游戏文件和不可处理原因 | `status` 为 `ok`，明细可解释 | 发现规则命中但未进入翻译时，先补规则或修外部规则，不凭工具猜语义 |
| `audit-coverage --game <游戏标题> --json` | 游戏标题 | 已导入规则和已保存译文 | 对比规则命中、统一文本清单、已保存译文和实际可写范围；规则命中但不可写、过期规则、缺译文和过期已保存译文都会作为错误报告 | `status` 为 `ok` | 先补规则、补译文或精确重置，不在审计失败时写进游戏文件 |
| `quality-report --game <游戏标题> --json` | 游戏标题 | 已有译文或翻译运行记录 | 判断是否可以写进游戏文件，并列出需要修的文本；其中 `quality_error_items` 是模型翻了但项目检查没通过的译文，`placeholder_risk_items` 是必须原样保留的游戏控制符风险，`overwide_line_items` 是某一行太长，`source_residual_items` 是疑似仍有源语言残留 | `status` 不是 `error` | 按报告明细修译文或规则；存在 `error` 时禁止继续写进游戏文件 |
...
| `export-pending-translations --game <游戏标题> --output <文件> --json` | 游戏标题、输出文件，可选 `--limit N` | 存在还没成功保存译文的文本，且需要抽样判断剩余问题，或剩余量适合手动处理 | 导出可填写的译文表；不传 `--limit` 时导出全部，传 `--limit N` 时分批或抽样导出 | 输出文件存在；若返回空结果，说明已经没有需要手动填写的文本 | 如果抽样显示仍是模型可以处理的问题，先确认规则检查通过，再回到 `translate` 继续降量 |
...
| `validate-source-residual-rules --game <游戏标题> --input <规则文件> --json` | 源文残留例外规则 JSON | 只在确需保留源语言片段时使用 | 校验位置例外和结构性例外；结构性例外只能遮蔽协议词，显示文本仍会继续检查 | `status` 为 `ok`；空规则只表示本轮没有源文残留例外要导入 | 修规则文件；不要关闭全局源文残留检测 |
...
| `verify-feedback-text --game <游戏标题> --input <反馈原文清单> --json` | 字符串数组或包含 `texts` 字符串数组的反馈原文清单 | 已把译文写进游戏文件 | 在真实 `data/*.json`、插件配置和插件源码中反查反馈原文是否仍存在，并按规则缺口、译文缺口、写入缺口或插件源码硬编码分类 | `status` 为 `ok` 且 `summary.occurrence_count=0` | 仍存在时按分类补规则、补译文、重新写进游戏文件，或把插件源码硬编码交给游戏侧专用处理流程 |
| `scan-plugin-source-text --game <游戏标题> --output <候选文件> --json` | 游戏标题、候选输出文件 | 反馈反查指向插件源码硬编码，或用户怀疑插件源码仍有源语言文本 | 只扫描常见插件源码 UI 文本 API 的字符串候选，不判断玩家是否可见，不自动改源码 | 输出文件存在，候选可人工审查 | 候选只作为后续游戏侧专用处理输入，不混入插件参数规则 |
| `write-back --game <游戏标题> --json` | 游戏标题、已保存译文、用户许可 | `quality-report --json` 无 error，且所有 warning 已确认不影响本轮写进游戏文件 | 写回游戏目录并输出摘要；默认不覆盖字体引用 | 命令返回 0 且 JSON 摘要可读 | 停止交付，按错误修质量或规则 |
| `write-back --game <游戏标题> --confirm-font-overwrite --json` | 游戏标题、已保存译文、用户明确允许覆盖字体 | `quality-report --json` 无 error，所有 warning 已确认不影响本轮写进游戏文件，且用户单独确认字体覆盖 | 写回游戏目录，并用配置的候选字体覆盖游戏数据里的字体引用；存在 `fonts/gamefont.css` 时同步备份和更新字体族入口；完整原始 `data` 备份和插件/样式原件用于后续对比还原 | 命令返回 0 且 JSON 摘要里 `font_copied=true` | 停止交付，先确认用户是否真的允许覆盖字体 |
| `restore-font --game <游戏标题> --json` | 游戏标题、`data_origin`、`plugins_origin.js`、`gamefont_origin.css`、候选覆盖字体名 | 用户要求还原项目覆盖过的字体引用 | 对比激活版和完整原始 `data` 备份、插件原件、字体样式原件，只把候选覆盖字体名替回同路径原件里的实际旧字体引用，不回滚译文 | 命令返回 0 且摘要可解释 | 若提示没有候选覆盖字体名，要求用户提供 `--replacement-font-path <字体文件>`；若缺少可对比的原始备份，先停止并说明无法按原件对比 |
| `cleanup-agent-workspace --workspace <工作区> --json` | 工作区目录 | `manifest.json` 存在 | 清理 CLI 生成的工作区文件 | 命令返回 0 | 缺 manifest 时手工确认后再清理 |

### 工作区 JSON 格式契约

- `mv-virtual-namebox-rules.json`：MV 专用规则文件，顶层必须是对象，格式为 `{ "rules": [...] }`，合法空结构是 `{ "rules": [] }`。每条规则必须声明 `name`、`pattern`、`speaker_group`、`speaker_policy` 和 `render_template`；有同一行正文时再声明 `body_group`。`pattern` 必须使用正则命名分组并完整匹配 `101` 后首条非空 `401` 的清理文本；`speaker_policy` 只能是 `translate`、`preserve`、`actor_name`。模板只能引用命名分组、`{speaker}` 和 `{body}`。禁止添加版本号、游戏标题、真实路径、说明字段或主流程不读取的字段。
- `placeholder-rules.json`：顶层必须是对象，格式为 `{正则表达式: 占位符模板}`。占位符模板必须生成形如 `[CUSTOM_NAME_1]` 的方括号占位符；推荐使用 `{index}`，例如 `[CUSTOM_PLUGIN_MARK_{index}]`。占位符规则作用于当前已经进入正文翻译集合的每一条文本，包括 RPG Maker 标准数据文本、已导入插件参数规则命中的文本、已导入事件指令规则命中的文本和已导入 Note 标签规则命中的文本；不会扫描未被规则纳入的原始插件参数、事件指令参数或 Note 标签字符串，也不会自动解析游戏私有协议语法。禁止写成 `{占位符名: 正则表达式}`，禁止把 RPG Maker 标准控制符当自定义规则硬写。
- `structured-placeholder-rules.json`：顶层必须是对象，格式为 `{ "paired_shell_rules": [...] }`，合法空结构是 `{ "paired_shell_rules": [] }`。每条规则必须声明 `name`、`pattern`、`translatable_group` 和 `protected_groups`；`pattern` 必须使用命名分组，`translatable_group` 指向继续交给模型翻译的分组，`protected_groups` 只保护固定协议外壳。模板必须生成形如 `[CUSTOM_NAME_1]` 的方括号占位符，普通正则规则不得与结构化规则的保护范围重叠。
- `terminology/field-terms.json`：这是“字段译名表”。顶层固定为术语类别对象，包括 `speaker_names`、`map_display_names`、角色、职业、技能、物品、装备、敌人、状态和系统类型术语。只填写已有 key 对应的 value；不改 key，不新增字段，不写 note，不把样本文件路径写入 value。它负责精确写回地图显示名、数据库名称、系统类型，以及 MZ 标准 `101.parameters[4]` 名字框等游戏字段。MV 的 `speaker_names` 是虚拟名字框说话人术语：由已导入 MV 虚拟名字框规则从每个对话块首条非空 `401` 正文识别，用于译名统一、正文翻译提示词命中，并在写进游戏文件时重建对应 `401` 说话人行或名字标签；不会写回 `101.parameters[4]`。字段译名表不允许空值；不确定项也必须给出当前最合理译名，并在最终报告说明风险。
...
- `plugin-rules.json`：顶层必须是数组，格式为 `[{plugin_index, plugin_name, paths}]`。`plugin_index` 必须是插件在 `plugins.json` 数组中的下标；`plugin_name` 必须与该下标插件的 `name` 完全一致；`paths` 是 JSONPath 字符串数组，必须使用括号路径语法并从 `$['parameters']` 开始，例如 `$['parameters']['message']` 或 `$['parameters']['items'][*]['name']`；禁止使用 `$.xxx` 点号路径。插件参数值如果是 JSON 字符串，规则不能停在外层参数字段，必须写到解析后的内部字符串叶子；可用 `plugin-json-string-leaf-candidates.json` 辅助找路径，但仍要人工判断是否玩家可见。没有可导入插件文本规则时使用空数组 `[]`。
- `event-command-rules.json`：顶层必须是对象，格式为 `{指令编码字符串: [{match, paths}]}`。`match` 是参数索引字符串到期望字符串值的对象；`paths` 是 `$['parameters']...` 路径数组；路径必须命中字符串叶子。没有过滤条件时 `match` 使用 `{}`。规则只依据 `event-commands.json` 里可见的数组位置和字符串叶子填写；参考文件里的插件命令结构只是用于说明判断方式，不能据此跳过当前文件检查。
- `note-tag-rules.json`：顶层必须是对象，格式为 `{data文件名或文件模式: [note标签名, ...]}`。只写候选里真实存在的精确标签名，不支持标签正则；文件名可用 `<地图文件模式>` 这类模式覆盖同类地图文件；空结果使用 `{}`，但必须说明已检查候选。禁止选择脚本、公式、资源名、ID、布尔/枚举、数值列表、资源引用、内部关联字段和纯系统标签。
...
- `source-residual-rules.json`：这是“允许保留源文的例外表”。顶层是 `{position_rules, structural_rules}`。`position_rules` 按文本在游戏里的内部位置放行，格式为 `{"<定位路径>": {"allowed_terms": ["<源文片段>"], "reason": "<原因>"}}`，定位键必须来自导出文件或质量检查报告，不允许自造。`allowed_terms` 是允许原样保留的源语言片段字符串数组。英文游戏默认允许少量 UI 缩写，但专名必须通过术语表或本规则显式放行。`structural_rules` 是结构性例外数组，每项包含 `pattern`、`allowed_terms`、`check_group` 和 `reason`；`structural_rules` 只能遮蔽协议词，显示文本仍会继续做分组源文残留检查。只在确认该片段确实不应翻译时使用；禁止用它掩盖整句漏翻，禁止在 `pending-translations.json` 内新增例外字段。
...
- 进入占位符规则编写、审查或修复前，必须读取 `references/translation-rule-examples.md` 的“占位符规则样例”小节；样例只帮助理解边界，不得照搬为当前游戏规则。
- 进入结构化占位符规则编写、审查或修复前，必须读取 `references/structured-placeholder-rules.md`；遇到固定协议外壳包住可翻译显示文本时，优先考虑结构化占位符规则，而不是用两条普通正则硬凑开头和结尾。
- 占位符系统的能力边界：它能在任意已进入正文翻译集合的文本里，把精确正则命中的协议片段替换成 `[CUSTOM_...]` 标记，让模型只看到应翻译的玩家可见文本；它不能让未被插件规则、事件指令规则或 Note 标签规则选中的字符串进入翻译，也不能替代理解游戏私有协议语法、拆分字符串叶子或判断某段自然语言是否可见。
- 占位符规则的来源范围：标准 data 文本天然进入正文翻译集合；插件参数、事件指令参数和 Note 标签只有先通过对应外部规则校验并导入，才会进入正文翻译集合。三类外部规则改变后，必须重新运行 `build-placeholder-rules`、`validate-placeholder-rules`、`scan-placeholder-candidates` 和 `import-placeholder-rules`。
- 占位符规则可以自由保护游戏私有协议壳、前后缀、插件命令关键字、参数标签、语音触发标记、名牌标记、自动替换触发标记、资源引用片段和其他必须原样保留的控制内容；前提是规则只遮蔽不可翻译协议片段，不能吞掉玩家可见文本。
- 复杂文本协议通用策略：任何来源的文本条目只要混合了必须原样保留的游戏协议和玩家可见文本，都按同一套占位符流程处理；来源可以是标准 data 字段、插件参数、事件指令参数或 Note 标签值，不得把策略绑定到某个指令编码、插件名、文件名或标签名。
- 判断一条混合文本能否交给模型时，先确认该来源已经被内置提取或对应外部规则纳入正文翻译集合，再看 `validate-placeholder-rules --sample <原文片段> --input <规则文件> --json` 的预览：去掉 `[CUSTOM_...]` 后仍应保留需要翻译的玩家可见文本；命令名、参数名、资源名、脚本、分隔符、标签壳、触发前后缀等不可翻译协议片段应被保护或本身不进入翻译。预览里如果模型看不到应翻译文本，或仍暴露会被模型改坏的协议语法，就不能导入或启动翻译。
...
- 校验和覆盖扫描都通过后：`import-placeholder-rules --game <游戏标题> --input <工作区>/placeholder-rules.json --json`。如果规则文件为空，只有当前候选确实为空时才允许加 `--confirm-empty` 导入；否则会报错。
- 如果存在结构化占位符规则，必须再用 `validate-structured-placeholder-rules --game <游戏标题> --input <工作区>/structured-placeholder-rules.json --json` 校验。
- 再用 `scan-structured-placeholder-candidates --game <游戏标题> --input <工作区>/structured-placeholder-rules.json --json` 扫描覆盖情况。
- 校验通过且覆盖风险确认后：`import-structured-placeholder-rules --game <游戏标题> --input <工作区>/structured-placeholder-rules.json --json`。如果规则文件为空，只有当前结构化候选确实为空时才允许加 `--confirm-empty` 导入；否则会报错。
...
- 裸无参数插件控制符可能直接贴着正文；工具不会自动猜协议边界。必须查插件源码或规则说明后，只保护已确认的协议本体，不能吞掉紧挨着的玩家可见正文；具体正反样例只在 `references/translation-rule-examples.md` 的“占位符规则样例”小节查看。
- 任何来源的单个文本字段或字符串叶子，都可能同时有控制词、参数名、资源名、分隔符、标签壳、触发前后缀和玩家可见文本。不要因为它“混有协议语法”就一概排除；也不要直接整段交给模型。只有在格式稳定、可用精确占位符规则保护所有不可翻译协议片段，并且预览里只留下玩家可见文本时，才允许把对应真实路径纳入相应规则或继续翻译。无法确认边界、格式不稳定或可见文本无法从协议壳中安全露出时，排除该文本并在报告中说明风险。
- 如果报告出现非 ASCII 右引号、全角括号或未闭合控制片段 warning，先用 Unicode code point 确认边界字符，再写精确规则；不要把 `\F3[66」「` 猜成 `\F3[66]「`。
- 具体正反样例只看 `references/translation-rule-examples.md`，不要在主流程中凭某个样例推导通用规则。
...
- 字段译名表负责写回地图显示名、数据库名称、系统类型，以及 MZ 标准 `101.parameters[4]` 名字框等游戏字段；正文术语表负责正文翻译提示词命中。两者不能互相替代。MV 的 `speaker_names` 虽然放在字段译名表里，但表示虚拟名字框说话人术语，只能写回对应 `401` 说话人行或名字标签，不表示可写回 `101.parameters[4]` 字段。
...
- 第二轮分析插件规则、事件指令规则和 Note 标签规则时，Agent 必须主动检查插件或脚本是否把名字框、地图显示名、数据库名称、系统类型或其他文本型字符串当作功能触发键使用。MV 说话人字段通常要从插件或文本规则里确认，不能默认补写 `101.parameters[4]`；按已导入 MV 虚拟名字框规则生成的 `speaker_names` 会服务术语统一、正文翻译提示词命中，并用于重建对应 `401` 虚拟名字框。
- 如果个别游戏插件依赖原始字符串触发语音、立绘、状态列表或其他功能，Agent 应在当前游戏目录新增或修改插件侧映射，让“中文显示值 -> 原始触发值”的关系留在游戏侧。
- 游戏侧映射只服务当前游戏，必须放在游戏目录内并说明影响范围；不得把例外游戏逻辑写进 A.T.T MZ 项目核心。
- 修改游戏侧映射前先说明发生了什么、影响什么、下一步怎么处理；修改后必须运行相关游戏目录检查或最小可验证流程，确认汉化字段仍能显示，插件触发也能按原始值工作。
...
- 导入：`import-plugin-rules --game <游戏标题> --input <工作区>/plugin-rules.json --json`。如果规则文件为空，只有当前插件候选确实为空时才允许加 `--confirm-empty` 导入；否则会报错。
...
- 导入：`import-event-command-rules --game <游戏标题> --input <工作区>/event-command-rules.json --json`。如果规则文件为空，只有当前事件指令候选确实为空时才允许加 `--confirm-empty` 导入；否则会报错。
...
- 导入：`import-note-tag-rules --game <游戏标题> --input <工作区>/note-tag-rules.json --json`。如果规则文件为空，只有当前 Note 标签候选确实为空时才允许加 `--confirm-empty` 导入；否则会报错。

## 10. 子代理规则

### 子代理任务处理方式确认

MV 第零轮完成并重新准备工作区后，主代理必须先向用户确认子代理任务处理方式，再启动第一轮术语候选；MZ 准备工作区完成后直接确认子代理任务处理方式。说明时必须让用户知道接下来会处理术语候选、插件规则、事件指令规则和 Note 标签规则；这些任务主要负责译名统一和玩家可见文本规则判断，不直接写进游戏文件。多项候选分析会消耗较多上下文和模型额度，额度有限时建议使用外部协作任务包。
...
进入术语候选派发或主代理合并审查前，必须读取 `references/translation-rule-examples.md` 的“术语候选样例”小节；样例只用于理解候选文件的保形原则和常见错误，不得替代当前游戏上下文判断。
...
进入第二轮三类外部规则派发或主代理验收前，必须读取 `references/translation-rule-examples.md` 的“三类外部规则样例”小节；样例只用于理解 schema、排除类别和空结果边界，不得把样例中的字段名、路径层级或文本类型当作当前游戏固定规则。

- `plugin-rules` 子代理：读取 `plugins.json` 和 `plugin-json-string-leaf-candidates.json`，只写 `plugin-rules.json`。
...
| 子代理 | 输入 | 逻辑 | 输出 |
| --- | --- | --- | --- |
| `plugin-rules` | `plugins.json`、`plugin-json-string-leaf-candidates.json` | 只选择插件参数里的玩家可见文本；外层 JSON 字符串字段不能直接当路径规则，必须判断解析后的内部字符串叶子；排除资源路径、文件名、脚本、枚举、布尔值、数字、颜色、坐标和内部标识 | 只写 `plugin-rules.json`，格式为 `[{plugin_index, plugin_name, paths}]`；无可见文本时输出 `[]` |
...
派发三类外部规则子代理时，`plugin-rules` 任务优先复制 `references/plugin-rules-agent-task.md` 的任务契约，`event-command-rules` 任务优先复制 `references/event-command-rules-agent-task.md` 的任务契约；如果内联任务单与 Skill references 不一致，以 Skill references 的输入、逻辑、输出和校验边界为准。

`plugin-rules` 子代理任务单：

```text
输入：读取 <工作区>/plugins.json 和 <工作区>/plugin-json-string-leaf-candidates.json。
逻辑：按插件逐项判断 parameters 内玩家可见文本；外层 JSON 字符串字段不能直接当路径规则，必须判断解析后的内部字符串叶子；排除资源路径、文件名、脚本、枚举、布尔值、数字、颜色、坐标和内部标识。
...
主代理派发子代理时，不能只概括“分析规则”。必须复制对应任务单，填入 `<发行版目录>`、`<工作区>`、`<游戏标题>`、输入文件、唯一可写文件和校验命令。需要具体正反样例时，只读取 `references/translation-rule-examples.md` 中对应小节，并提醒子代理：样例只说明判断方式，不是当前游戏固定答案。

## 11. 翻译失败处理

- `translate` 返回 0 只表示本轮命令正常结束，不代表所有文本都已经成功保存译文。
- “还没成功保存译文的文本”和少量或中等规模的“模型翻了但项目检查没通过的译文”都可能是可续跑状态。先看翻译进度报告和质量检查报告；只要剩余数量明显下降，且没有新的规则性事故，就继续跑同一个 `translate` 命令。
- 第 N 轮续跑必须形成闭环：每轮 `translate` 结束后，都运行 `translation-status --game <游戏标题> --json` 和 `quality-report --game <游戏标题> --json`，记录本轮开始数量、当前剩余数量、检查没通过的译文数量和主要错误类型。数量明显下降时继续下一轮；连续多轮同类失败不下降时，才停止盲目重跑。
- 停止盲目重跑时，必须向用户说明：当前剩余数量、主要错误类型、最近几轮是否下降、是否可换模型、是否需要改规则、是否已经适合由 Agent 手动填写译文表。禁止只说“失败很多”就直接导出全部手动处理。
- 小批量后如果有模型接口失败、译文检查没通过、游戏控制符可能被改坏，先排查，不继续全量翻译。
- 要查看最新一轮“模型翻了但项目检查没通过”的全部错误明细，先运行质量检查报告：`quality-report --game <游戏标题> --json`。报告里的 `quality_error_items` 表示模型翻了但项目检查没通过的译文；`placeholder_risk_items` 表示必须原样保留的游戏控制符可能被改坏；`overwide_line_items` 表示某一行太长，游戏窗口放不下；`source_residual_items` 表示中文译文里还有疑似没翻的源语言文本。
...
- 如果质量检查报告只提示还有文本没成功保存译文，但没有可修复明细，不要立刻导出全部手动填写。先按第 N 轮续跑继续降量；只有剩余数量已经适合手动处理，或连续多轮同类失败不下降并已完成上述报告，才使用 `export-pending-translations --game <游戏标题> --output <工作区>/pending-translations.json --json` 一次导出全部剩余文本。
- 如果只想抽样判断剩余问题，或剩余数量适合分批手动处理，使用 `export-pending-translations --game <游戏标题> --limit N --output <工作区>/pending-translations.json --json`。不传 `--limit` 时导出全部剩余文本，但全量导出仍必须满足“已经降到适合手动处理或多轮不下降”的前置条件。
...
- `quality-report --json` 的所有 `warning` 已逐项确认，不影响本轮写进游戏文件，并已在阶段报告中说明。
- 占位符规则已覆盖当前游戏候选。
- 字段译名表、正文术语表、插件规则、事件指令规则、Note 标签规则、普通占位符规则和结构化占位符规则已导入；如果确实没有对应内容，必须通过导入命令的 `--confirm-empty` 保存空结果确认。
...
- 规则导入 JSON 返回 `deleted_translations_backed_up` warning 时，说明本次规则变化清理了不再属于当前规则范围的已保存译文；备份文件路径在 `summary.deleted_translation_backup_path` 和 `details.deleted_translation_backup.path`。如果确认导错，先重新导入正确规则，再用 `import-manual-translations --game <游戏标题> --input <备份文件> --json` 恢复这些译文。
- `placeholder_rules_invalid`：优先检查是否把 `{正则表达式: 占位符模板}` 写反、模板是否能生成 `[CUSTOM_NAME_1]`、正则是否能编译。
- `plugin_rules_invalid`：优先检查顶层是否是数组、`plugin_index` 是否是 `plugins.json` 数组下标、`plugin_name` 是否与该下标插件名一致、JSONPath 是否从 `$['parameters']` 开始、是否误用了 `$.xxx` 点号路径、路径是否命中字符串叶子；如果错误提示提到 JSON 字符串容器，先看 `plugin-json-string-leaf-candidates.json` 并把路径改到内部字符串叶子。
...
- `source_residual_rules_invalid`：优先检查顶层是否是 `{position_rules, structural_rules}`；`position_rules` 的定位键必须来自当前文本在游戏里的内部位置，`allowed_terms` 必须是非空字符串数组且片段出现在当前条目原文或译文中，`reason` 必须非空；`structural_rules` 必须包含可编译的 `pattern`、非空 `allowed_terms`、存在于正则里的 `check_group` 和非空 `reason`。
