---
name: att-mz-release
description: 执行 A.T.T MZ 发行版的 RPG Maker MV/MZ 游戏翻译流程：注册游戏、准备工作区、分析术语和文本规则、导入规则、调用模型翻译、检查译文、手动修复、写进游戏文件，并根据试玩反馈持续补漏。
---
...
- `<发行版目录>` 是 A.T.T MZ 发行包目录。发行版默认命令是 `.\att-mz.exe --agent-mode <命令> ...`。
