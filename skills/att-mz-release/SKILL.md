...
| `validate-mv-virtual-namebox-rules --game <游戏标题> --input <规则文件> --json` | MV 虚拟名字框规则 JSON | 已阅读 `references/mv-virtual-namebox-rules.md`；规则文件存在 | 校验正则命名分组、模板字段、同一候选多规则命中和源文重建 | `status` 不是 `error`；空规则只允许 warning | 修 `mv-virtual-namebox-rules.json` 后重跑；MZ 不使用该命令 |
| `import-mv-virtual-namebox-rules --game <游戏标题> --input <规则文件> --json` | 已校验 MV 虚拟名字框规则 | validate 无 error；空规则必须加 `--confirm-empty` | 保存为当前 MV 游戏虚拟名字框规则，供 `speaker_names`、正文提取和写回使用 | `status` 为 `ok`，或空规则时只有 warning | 导入后重新运行 `prepare-agent-workspace`，让第一轮术语表使用当前规则 |
...
| `write-back --game <游戏标题> --confirm-font-overwrite --json` | 游戏标题、已保存译文、用户明确允许覆盖字体 | `quality-report --json` 无 error，所有 warning 已确认不影响本轮写进游戏文件，且用户单独确认字体覆盖 | 写回游戏目录，并用配置的候选字体覆盖游戏数据里的字体引用；存在 `fonts/gamefont.css` 时同步备份和更新字体族入口；原件留档用于后续对比还原 | 命令返回 0 且 JSON 摘要里 `font_copied=true` | 停止交付，先确认用户是否真的允许覆盖字体 |
