...
`file` 只能是 `js/plugins` 直接 `.js` 文件名，不能包含目录分隔符。`selectors` 必须来自 `plugin-source-ast-map.json`，禁止手写或改写。低风险时允许保持空数组；高风险时必须先询问用户，用户肯定后导出 AST 地图并导入规则，用户明确确认不需要时才允许空规则。
