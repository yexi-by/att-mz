...
确认风险后的未覆盖候选仍会在 `doctor`、`text-scope`、`audit-coverage` 和 `quality-report` 中作为 warning 可见，但不会阻止正文翻译或写进游戏文件。旧版样本确认不再作为当前确认依据；候选范围变化或旧确认过期时，必须重新审查并导入当前规则。确认风险不是允许翻坏协议片段；如果模型或人工译文删除、改写未覆盖疑似控制符，保存前校验、质量检查或写文件前检查必须报 error。
