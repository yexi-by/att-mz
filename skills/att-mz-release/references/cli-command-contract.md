...
本文件是发行版翻译流程 Agent 的执行契约，只记录如何安全调用 A.T.T MZ CLI、如何判断阶段结果、失败后回到哪一步。源码仓库中的 `docs/wiki/cli.md` 只供维护者理解当前实现，不是发行版 Agent 执行依据；发行版流程不要读取源码、数据库表结构或维护者 wiki 来替代本文件、CLI 实际 JSON 输出或当前工作区文件。
...
- 所有工作区 JSON、临时脚本、手动填写译文表、规则文件和报告都按 UTF-8 读写。Windows 终端乱码时先设置 UTF-8 后重跑命令，不要基于乱码修改规则或译文。发行版流程不得要求用户安装 Python、Node.js、Rust、uv 或 maturin；只有当前环境已经具备且用户允许时，才用临时脚本辅助处理工作区文件。
