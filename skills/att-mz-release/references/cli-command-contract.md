...
| `text-scope --game <游戏标题>` | 查看统一文本范围和规则来源 | `status` 为 `ok`；加 `--include-write-probe` 时只标记索引可写状态，`write_back_probe_executed=false` | 发现规则命中但不可翻译时先修规则 |
| `audit-coverage --game <游戏标题>` | 对比规则命中、译文和当前文本范围 | `status` 为 `ok`；加 `--include-write-probe` 时只标记索引可写状态，`write_back_probe_executed=false` | 补规则、补译文或精确重置 |
...
| `quality-report --game <游戏标题>` | 使用索引和已保存译文做普通质量报告；默认不执行写入可行性探针；索引缺失或过期时会先自动重建并在 `summary.text_index_status` 标明 `cold_rebuilt` 或 `stale_rebuilt` | `status` 不是 `error`；需要 Rust 写回级检查时加 `--include-write-probe`，成功执行后 `write_back_probe_executed=true`、`write_back_probe_mode=rust_write_gate` | 按明细修译文或规则；有 error 禁止写回 |
| `export-quality-fix-template --game <游戏标题> --output <文件>` | 导出检查没通过译文的修复表 | 输出文件存在，数量可解释；加 `--include-write-probe` 只标记请求和索引可写状态，不额外执行写回级检查 | 只改中文译文行后导入 |
| `export-pending-translations --game <游戏标题> --output <文件>` | 导出还没成功保存译文的文本表 | 输出文件存在；可加 `--limit N`；加 `--include-write-probe` 只标记请求和索引可写状态，不额外执行写回级检查 | 抽样显示仍适合模型时回到翻译 |
