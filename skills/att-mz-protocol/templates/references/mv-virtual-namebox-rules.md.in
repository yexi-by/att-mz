...
`pattern` 必须同时能被 Python `re` 和 Rust `fancy-regex` 编译。命名分组统一使用 Python 风格 `(?P<name>...)`，`speaker_group` 和 `body_group` 必须对应这种写法；不要使用其它正则方言的命名分组，也不要使用只在某一边可用的语法如 `(?a:...)`。校验和导入阶段会提前预检，当前游戏已保存规则不满足当前契约时 `doctor`、`text-scope`、`audit-coverage` 和 `quality-report` 会返回 `mv_virtual_namebox_rules_invalid`。
