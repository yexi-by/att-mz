...
空结构 `{ "paired_shell_rules": [] }` 必须加 `--confirm-empty` 才能导入；如果仍有未覆盖候选，导入会保存“已审查但不写结构化规则”的确认状态并返回 warning。非空规则导入后仍有未覆盖候选时，也会保存“剩余风险已确认”的状态并持续 warning。旧版样本确认不再作为当前确认依据；候选范围变化或旧确认过期时，必须重新审查并导入当前规则。不要为了通过扫描而编造结构化规则；无法确认外壳和可翻译分组时，优先报告风险或补外部文本规则。
