...
| 插件源码文本 | 处理少见的 `js/plugins` 源码硬编码显示文本 | `plugin-source-risk-report.json`、AST 地图、只读插件源码、源码规则草稿 | 已导入插件源码规则或高风险空结果确认 | 高风险时先询问用户，肯定后用 AST 地图筛选并只读源码交叉验证，再 validate/import 成功 | 未获用户确认、高风险未处理、selector 失效、试图扫描 plugins 外目录 |
...
7. 若 `plugin-source-risk-report.json` 为高风险，必须先询问用户；用户肯定后读取插件源码任务契约，导出 AST 地图，必要时只读对应插件源码做语义交叉验证，再填写并导入插件源码规则；用户未确认时停止正文翻译。
