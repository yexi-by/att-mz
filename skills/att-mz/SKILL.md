...
本 Skill 是面向 A.T.T MZ 发行版文件夹的翻译任务执行协议，不是项目说明书。使用本 Skill 的 Agent 默认只接触发行版里的可执行文件、配置、运行数据、Skill 和工作区文件，不依赖 Rust 源码或仓库结构。按阶段执行，遇到不能继续的错误就停下报告，不要靠猜。第一次写进游戏文件得到的是可试玩汉化结果，不是百分百完成承诺；真正稳定的汉化版本需要 Agent 根据用户试玩反馈继续查缺补漏。

## 按需参考资料

- RMMZ 引擎常识、标准控制符、事件指令、数据库字段、插件与 Note 标签的基础知识放在 `references/rmmz-world-knowledge.md`。
...
- 遇到裸 `\N`、`\V`、`\C`、`\I`、`\PX`、`\PY`、`\FS` 这类疑似缺参数片段时，先查参考文档再下结论；不要直接当作完整标准 RMMZ 控制符。
...
- `<发行版目录>`：用户解压后的 A.T.T MZ 运行目录，不是源码仓库。目录里至少应有当前系统的可执行文件、`setting.toml` 或示例配置、`data/`、`skills/`；`logs/`、`outputs/` 可以由 CLI 运行时生成。翻译任务只允许运行发行版内 CLI、读取发行版内 Skill、配置和 CLI 输出；如果怀疑工具本身有缺陷，停止翻译并报告工具问题。翻译任务中禁止在这里写临时脚本、中间 JSON、抽样报告和手动填写译文表。
- `<att-mz>`：发行版内当前系统对应的可执行文件。Windows 通常是 `<发行版目录>/att-mz.exe`，Linux/macOS 通常是 `<发行版目录>/att-mz`。如果发行包按三端拆成平台子文件夹，进入当前平台子文件夹后，它就是本 Skill 所说的 `<发行版目录>`。
- `<发行版目录>/data`：保存已注册游戏状态、已保存译文记录和运行期数据。不要手工编辑、搬运或删除这里的内部文件；升级发行版时应按用户选择整体保留或备份迁移。
...
- 翻译任务中，临时脚本不得直接操作 `<发行版目录>/data` 下的内部文件、Rust 内部模块或游戏数据文件。业务数据进出必须走发行版 CLI。
- 如果用户要求开发或修改 A.T.T MZ 工具本身，上面“只走 CLI”的翻译任务限制不适用，但必须明确这是开发任务，不是翻译任务。

## 2. 固定命令习惯

- 进入 `<发行版目录>` 后运行命令，确保 CLI 能从同一目录读取配置、运行数据和 Skill。
- 默认使用：`<att-mz> <命令> ...`。下文写成 `doctor --json` 这类命令时，都表示在前面加上 `<att-mz>` 后执行。
...
- 全局参数放在子命令前，例如 `<att-mz> --agent-mode doctor --no-check-llm --json`。
...
- `<发行版目录>` 可进入，并能执行 `<att-mz> --help`。
- `<发行版目录>/setting.toml` 已存在，或发行版提供可复制的示例配置并且用户允许初始化。
- `<发行版目录>/skills/att-mz/SKILL.md` 存在；任务执行以当前这份 Skill 为准。
...
主代理可以在 `<工作区>` 内写临时脚本分析这些文件。A.T.T MZ 只关心最终是否通过 CLI 校验并保存为当前游戏规则。

### 只按任务文件和命令结果工作

- 翻译任务中，只按 CLI 输出、工作区 JSON、游戏目录和用户明确提供的信息判断；禁止靠工具源码或程序内部数据猜规则格式。
...
- 不需要知道：Rust 源码、程序内部数据、模型提示词怎样生成、占位符怎样恢复、译文怎样写进游戏文件、已保存译文记录怎样存放。
...
| --- | --- | --- | --- | --- | --- |
| `doctor --no-check-llm --json` | `<发行版目录>`、本地配置 | 可进入发行版目录 | 检查发行版运行环境；缺失 `data/db` 时应自愈创建 | `status` 不是 `error` | 按 `errors` 修环境；不启动翻译 |
...
| `validate-plugin-rules --game <游戏标题> --input <规则文件> --json` | 插件规则 JSON | `plugins.json` 已分析 | 校验插件名、插件哈希和 JSONPath 命中字符串叶子 | `status` 为 `ok`，或空规则 warning 已确认 | 修 `plugin-rules.json`，不读工具源码猜路径 |
...
- 完整标准 RMMZ 控制符如果被报告为未覆盖，先停下报告工具异常，不要硬凑规则；如果只是裸 `\N` 这类缺参数片段，先按 `references/rmmz-world-knowledge.md` 判断它是否属于不完整控制符、插件自定义协议或源文本异常。
...
派发三类外部规则子代理时，直接复制本节内置任务单，并填入当前发行版目录、工作区、游戏标题、输入文件、唯一可写文件和校验命令。不要要求子代理阅读源码仓库里的参考文档；发行版 Skill 必须能单独支撑任务执行。
