...
11. 查看翻译进度报告 `translation-status --json` 和质量检查报告 `quality-report --json`，记录剩余数量、检查没通过的译文类型和是否出现规则性事故。
...
13. 只有多轮续跑后剩余量适合手动处理，或连续多轮同类失败不再下降并已向用户说明原因时，才导出手动填写译文表。需要一次导出全部剩余文本时，用 `export-untranslated-translations`；只想抽样或分批时，用 `export-pending-translations --limit N`；填写中文译文行后，用 `import-manual-translations` 交回项目检查并保存。
...
15. `quality-report --json` 没有 `error` 错误，所有 `warning` 已逐项确认不会影响本轮写进游戏文件，并且用户允许写回后，执行 `write-back --game <游戏标题> --json`。普通写回不会覆盖字体；只有用户另外明确允许覆盖字体时，才执行 `write-back --game <游戏标题> --confirm-font-overwrite --json`。
...
| `export-untranslated-translations --game <游戏标题> --output <文件> --json` | 游戏标题、输出文件 | 存在还没成功保存译文的文本，且已经经过第 N 轮续跑降量，剩余量适合手动处理，或连续多轮同类失败不下降并已向用户说明原因 | 一次导出全部还没成功保存译文的原文，生成可填写的译文表 | 输出文件存在 | 若 warning 为空结果，说明已经没有需要手动填写的文本；如果剩余数量仍很大且每轮仍在下降，回到 `translate` 继续降量 |
| `export-pending-translations --game <游戏标题> --limit N --output <文件> --json` | 游戏标题、数量、输出文件 | 存在还没成功保存译文的文本，且需要抽样判断剩余问题，或剩余量适合分批手动处理 | 分批或抽样导出可填写的译文表；不传 `--limit` 时导出全部，但全量导出仍必须满足手动处理前置条件 | 输出文件存在 | 若 warning 为空结果，说明已经没有需要手动填写的文本；如果抽样显示仍是模型可继续处理的问题，回到 `translate` 继续降量 |
...
- 如果质量检查报告只提示还有文本没成功保存译文，但没有可修复明细，不要立刻导出全部手动填写。先按第 N 轮续跑继续降量；只有剩余数量已经适合手动处理，或连续多轮同类失败不下降并已完成上述报告，才使用 `export-untranslated-translations --game <游戏标题> --output <工作区>/pending-translations.json --json` 一次导出全部剩余文本。
