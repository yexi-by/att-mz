...
| `import-structured-placeholder-rules --game <游戏标题> --input <规则文件> --json` | 已校验的结构化规则文件 | validate 已通过，覆盖风险已确认 | 保存为当前游戏结构化占位符规则 | `status` 为 `ok` 或只有可接受 warning | 回到 validate/scan 修规则，不绕过 CLI |
...
| `import-note-tag-rules --game <游戏标题> --input <规则文件> --json` | 已校验 Note 标签规则 | validate 无 error | 导入标准 `data/*.json` 的 `note` 标签文本规则 | `status` 为 `ok`，或空规则 warning / `deleted_translations_backed_up` warning 已确认 | 回到 validate 修规则；若导错，先重新导入正确规则，再用备份文件恢复译文 |
...
| `translate --game <游戏标题> --max-batches 1 --json` | 游戏标题、模型配置 | 工作区已校验并导入 | 小批量试跑正文翻译 | 命令返回 0，质量报告没有新增 `error`，warning 已判断可继续 | 看 status 和 quality-report，不盲目全量 |
...
| `quality-report --game <游戏标题> --json` | 游戏标题 | 已有译文或翻译运行记录 | 判断是否可以写进游戏文件，并列出需要修的文本；其中 `quality_error_items` 是模型翻了但项目检查没通过的译文，`placeholder_risk_items` 是必须原样保留的游戏控制符风险，`overwide_line_items` 是某一行太长，`source_residual_items` 是疑似仍有源语言残留 | `status` 为 `ok`，或 `status` 为 `warning` 且所有 warning 已确认不影响下一步 | 按报告明细修译文或规则；存在 `error` 时禁止继续写进游戏文件 |
...
| `export-pending-translations --game <游戏标题> --output <文件> --json` | 游戏标题、输出文件，可选 `--limit N` | 存在还没成功保存译文的文本，且需要抽样判断剩余问题，或剩余量适合手动处理 | 导出可填写的译文表；不传 `--limit` 时导出全部，传 `--limit N` 时分批或抽样导出 | 输出文件存在 | 若 warning 为空结果，说明已经没有需要手动填写的文本；如果抽样显示仍是模型可继续处理的问题，回到 `translate` 继续降量 |
...
| `validate-source-residual-rules --game <游戏标题> --input <规则文件> --json` | 源文残留例外规则 JSON | 只在确需保留源语言片段时使用 | 校验位置例外和结构性例外；结构性例外只能遮蔽协议词，显示文本仍会继续检查 | `status` 为 `ok`，或空规则 warning 已确认 | 修规则文件；不要关闭全局源文残留检测 |
...
- 校验通过且覆盖风险确认后：`import-structured-placeholder-rules --game <游戏标题> --input <工作区>/structured-placeholder-rules.json --json`。
