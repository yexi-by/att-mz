...
5. MV 第零轮：如果 `manifest.json` 的 `engine_kind` 是 `mv`，主代理必须先读取 `references/mv-virtual-namebox-rules.md` 和 `<工作区>/mv-virtual-namebox-candidates.json`，亲自填写 `<工作区>/mv-virtual-namebox-rules.json`，运行 `validate-mv-virtual-namebox-rules --game <游戏标题> --input <工作区>/mv-virtual-namebox-rules.json --json`，再运行 `import-mv-virtual-namebox-rules --game <游戏标题> --input <工作区>/mv-virtual-namebox-rules.json --json`；确认空规则时文件为 `{"rules":[]}` 且导入命令加 `--confirm-empty`。导入后必须重新运行 `prepare-agent-workspace`，保证第一轮 `speaker_names` 按虚拟名字框规则生成。MZ 游戏跳过本步骤。
