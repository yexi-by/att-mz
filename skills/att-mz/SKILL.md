...
5. MV 第零轮：如果 `manifest.json` 的 `engine_kind` 是 `mv`，主代理必须先读取 `references/mv-virtual-namebox-rules.md` 和 `<工作区>/mv-virtual-namebox-candidates.json`，亲自填写 `<工作区>/mv-virtual-namebox-rules.json`，运行 `validate-mv-virtual-namebox-rules --game <游戏标题> --input <工作区>/mv-virtual-namebox-rules.json --json`，逐条审查 `details.newly_matched_candidates` 中的 `text`、`following_lines`、`matching_rules` 和 `matches`；任何不是说话人名字框的样本都必须通过收紧规则排除并重新校验。确认校验无 error 且新增命中样本已经审查后，再运行 `import-mv-virtual-namebox-rules --game <游戏标题> --input <工作区>/mv-virtual-namebox-rules.json --json`；确认空规则时文件为 `{"rules":[]}` 且导入命令加 `--confirm-empty`。导入后必须重新运行 `prepare-agent-workspace`，保证第一轮 `speaker_names` 按虚拟名字框规则生成。MZ 游戏跳过本步骤。
...
- 如果用户明确要求开发、修复或审查 A.T.T MZ 项目本身，或同一合法工作区文件反复触发无法解释的 CLI 错误、Skill/CLI 契约互相矛盾、命令帮助与文档不一致，可以升级为工具排障任务。
- 工具排障任务允许阅读 A.T.T MZ 源码、测试和命令注册逻辑来定位原因；但业务数据进出仍必须走 CLI、工作区 JSON 和游戏目录文件，禁止直接手改数据库或绕过校验写进游戏文件。
...
主代理派发子代理时，不能只概括“分析规则”。必须复制对应任务单，填入 `<项目目录>`、`<工作区>`、`<游戏标题>`、输入文件、唯一可写文件和校验命令。需要具体正反样例时，只读取 `references/translation-rule-examples.md` 中对应小节，并提醒子代理：样例只说明判断方式，不是当前游戏固定答案。
