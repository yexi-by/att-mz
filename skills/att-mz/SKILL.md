...
8. 主代理基于当前会进入正文翻译的完整文本集合，亲自生成、审查、覆盖扫描、校验并导入普通占位符规则；如存在“协议外壳固定、中间文本可翻译”的混合文本，还必须按结构化占位符规则流程单独校验并导入。
9. `validate-agent-workspace --game <游戏标题> --workspace <工作区> --json`。
10. 小批量翻译：`translate --game <游戏标题> --max-batches 1 --json`。
11. 查看翻译进度报告 `translation-status --json`、统一文本清单 `text-scope --json`、覆盖审计 `audit-coverage --json` 和质量检查报告 `quality-report --json`，记录剩余数量、当前规则命中范围、检查没通过的译文类型和是否出现规则性事故。
12. 稳定后进入第 N 轮续跑：每轮执行 `translate --game <游戏标题> --json`，随后重新查看翻译进度报告和质量检查报告。只要剩余数量明显下降，且没有新的规则性事故，就继续下一轮，直到没有“还没成功保存译文的文本”，或剩余量已经适合由 Agent 手动填写译文表。
13. 只有多轮续跑后剩余量适合手动处理，或连续多轮同类失败不再下降并已向用户说明原因时，才导出手动填写译文表。统一使用 `export-pending-translations`；需要抽样或分批时追加 `--limit N`，不传 `--limit` 时一次导出全部剩余文本；填写中文译文行后，用 `import-manual-translations` 交回项目检查并保存。
...
15. `audit-coverage --json` 和 `quality-report --json` 都没有 `error` 错误，所有 `warning` 已逐项确认不会影响本轮写进游戏文件，并且用户允许写回后，执行 `write-back --game <游戏标题> --json`。普通写回不会覆盖字体；只有用户另外明确允许覆盖字体时，才执行 `write-back --game <游戏标题> --confirm-font-overwrite --json`。
16. 写回完成后，向用户说明这是第一版可试玩汉化结果；请用户运行游戏，重点反馈漏翻、误翻、术语不统一、窗口显示异常、剧情语气不自然、按钮菜单看不懂、插件界面仍是源语言文本、图片文字未处理等问题。用户给出反馈原文清单后，必须用 `verify-feedback-text --game <游戏标题> --input <反馈原文清单> --json` 反查真实文件。
...
`prepare-agent-workspace` 会优先把 CLI 已保存到当前游戏状态里的字段译名表、正文术语表、插件规则、事件指令规则、Note 标签规则、普通占位符规则和结构化占位符规则回填到工作区。新游戏或未保存规则时，工作区仍会给出空对象或候选草稿，供主代理分析填写。占位符候选和草稿只是前置参考，最终规则必须在字段译名表、正文术语表和三类外部规则保存后重新生成或逐条确认。
...
- 第二轮分析插件规则、事件指令规则和 Note 标签规则时，Agent 必须主动检查插件或脚本是否把名字框、地图显示名、数据库名称、系统类型或其他文本型字符串当作功能触发键使用。MV 说话人字段通常要从插件或文本规则里确认，不能默认补写 `101.parameters[4]`；CLI 默认识别出的 MV `speaker_names` 会服务术语统一、正文翻译提示词命中，并用于重建对应 `401` 虚拟名字框。
