...
- `tests/test_write_back_transactions.py`、`tests/test_rmmz_post_write_audit.py` 和 `tests/test_rmmz_file_transaction.py` 覆盖提取后的写回事务、写后审计和文件替换边界；写回计划内部归 Rust/native owner 测试。
