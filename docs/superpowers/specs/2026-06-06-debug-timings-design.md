...
`--debug` 只覆盖 debug 总开关，不覆盖 `debug.timings.enabled`，也不强制开启全部 debug 子功能。计时功能由 `debug.timings.enabled`、`ATT_MZ_DEBUG_TIMINGS`、`--debug-timings` 或 `--no-debug-timings` 决定。只有 debug 总开关和 timings 子功能同时开启时，才启用统一计时诊断。
...
- 配置 schema 支持 `[debug]` 和 `[debug.timings]`。
- CLI、环境变量、配置文件和默认值的优先级。
- `--debug` 不强制开启全部子功能，`--debug-timings` 和 `--no-debug-timings` 只覆盖计时功能。
...
2. 增加 `DiagnosticsContext`、`TimingCollector` 和 no-op 实现。
3. 在 CLI 入口创建和 finalize 诊断上下文。
4. 在报告层统一注入 `summary.diagnostics`。
5. 迁移并删除普通报告中的旧计时字段。
6. 接入 P0 主路径关键阶段和现有 Rust 计时桥接。
7. 接入 P1 命令 L0/L1。
8. 补齐安全测试和普通模式无泄漏测试。
