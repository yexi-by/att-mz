...
为降低理解成本和避免重复解析逻辑，CLI、环境变量和配置文件的优先级合并必须通过一个统一解析器完成。新增 debug 子功能时，只能在该解析器登记功能名、默认值、环境变量名和 CLI 覆盖字段，不得在各命令里重新实现优先级判断。

debug 配置加载必须是轻量路径。CLI 入口只读取 `setting.toml` 中的 `[debug]` 配置域并做 debug schema 校验，不加载完整业务 `Setting`，不读取提示词文件，不依赖游戏源语言。完整业务配置仍由现有命令流程按需加载。
...
- `current_diagnostics()` 或同名门面：通过 `contextvars.ContextVar` 读取当前 CLI 运行的诊断上下文。

业务代码接入方式：

```python
with current_diagnostics().stage("quality.read_rules"):
...
```python
current_diagnostics().record_timing("quality.native_quality", stage_timings["native_quality"])
current_diagnostics().counter("quality.native_quality_payload_item_count", len(native_quality_items))
```
...
for name, value in plan.timings_ms.items():
    current_diagnostics().record_timing(f"write_back.rust_plan.{name}", value)
```

边界规则：

- `DiagnosticsContext` 由 CLI 入口创建，最终也由 CLI 入口 finalize。
- 诊断上下文通过 `contextvars.ContextVar` 绑定到本次 CLI 运行，避免把 cross-cutting 参数一路加进业务 API。
- 禁止使用普通全局可变单例保存运行态诊断上下文；测试必须能隔离当前上下文。
...
所有 stdout JSON 都必须经过统一报告出口或统一注入 helper。不能让 `write_report_outputs`、`print(report.to_json_text())` 和顶层错误报告各自实现一份 diagnostics 注入逻辑。
...
P1 第一版不为了打点重写业务流程，也不为了拿一个计数引入额外全量扫描。若关键计数没有稳定来源，先记录命令级和已有阶段级信息，等该命令进入 P0 或真实性能问题出现时再加深覆盖。
...
为避免 debug runtime 本身成为新的复杂度来源，第一版必须遵守：

- debug 配置只在统一解析器里合并优先级。
- 业务函数签名不因诊断上下文大面积膨胀。
- P0/P1 打点必须优先放在已有阶段边界、外部接口边界和重资源调用边界。
- 不为了记录计数而新增额外全量扫描或重复数据库查询。
- 诊断字段命名使用 `domain.stage.metric` 风格，不使用中文自然语言 key。
- 普通报告中的 diagnostics 注入只允许走统一报告出口或统一 helper。
- 新增 debug 子功能必须复用 debug 配置域、统一解析器、统一输出治理，不新增第二套开关体系。
...
- CLI 入口加载 debug 配置时不加载完整业务 `Setting`，不读取提示词文件，不依赖目标游戏源语言。
- 诊断上下文通过 `contextvars` 隔离；连续运行或并发异步任务不会串写同一个诊断上下文。
- stdout JSON 的 diagnostics 注入通过统一出口或统一 helper 生效，直接打印报告的命令也不能漏注入或重复注入。
...
1. 增加 debug 专用轻量配置 schema、环境变量读取和 CLI 覆盖解析。
2. 增加统一 debug 功能优先级解析器，覆盖 logging 与 timings。
3. 将现有 `--debug` 日志行为迁入统一 debug logging 配置。
4. 增加 `DiagnosticsContext`、`TimingCollector`、`contextvars` 当前上下文和 no-op 实现。
5. 在 CLI 入口创建、绑定、finalize 诊断上下文。
6. 在报告层提供统一 diagnostics 注入 helper，并让所有 stdout JSON 出口复用。
7. 迁移并删除普通报告中的旧计时字段。
8. 接入 P0 主路径关键阶段和现有 Rust 计时桥接。
9. 接入 P1 命令 L0/L1，禁止为了计数新增额外全量扫描。
10. 补齐安全测试、日志收口测试、上下文隔离测试和普通模式无泄漏测试。
