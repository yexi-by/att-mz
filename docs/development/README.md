...
5. 涉及 RPG Maker VX / VX Ace / RGSS 系列引擎适配时读 [RGSS 引擎支持调研记录](rgss-engine-notes.md)。
...
10. 准备大范围破坏性重构前读 [超重型破坏性重构前 Review 指南](pre-heavy-refactor-review.md)。
...
| [RGSS 引擎支持调研记录](rgss-engine-notes.md) | 未来 RGSS 引擎适配层 | VX / VX Ace 样本为何需要归档解包、Ruby Marshal 反序列化和 Ruby 脚本解析 |
...
| [超重型破坏性重构前 Review 指南](pre-heavy-refactor-review.md) | 全项目结构、主流程、事实来源、公共抽象、测试和文档契约 | 大范围重构前先审哪些结构债、旧路径和重复逻辑 |
