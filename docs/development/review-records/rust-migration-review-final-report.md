...
动态样本 `テイルズ・テイルス` 副本包含 `data_origin`，`add-game` 正确拒绝了非干净目录，但 JSON 输出 code 是 `unexpected_error`，而不是可恢复的业务错误。
...
- 动态验证中 `add-game --json` 返回 `unexpected_error`。
...
`scripts/benchmark_rebuild_active_runtime.py` 默认样本路径写死 `D:\h-game\测试样本\...`。这不适合作为长期仓库脚本默认值，也和示例脱敏、配置分离要求冲突。
...
- `D:\h-game\测试样本` 顶层目录本身不是游戏根。
- `D:\h-game\测试样本\未汉化` 一层扫描未发现标准 MV/MZ 根目录。
- `D:\h-game\测试样本\已汉化` 中发现多个 MV/MZ 候选。
...
- `add-game --json` 成功，游戏标题 `頽廃のシスター`。
- `ATT_MZ_RUST_THREADS=4`。
- 动态命令结果：

| 命令 | 退出码 | 结果 | 耗时 |
| --- | ---: | --- | ---: |
| `translation-status --json` | 0 | warning `translation_run_missing` | 1241ms |
| `scan-placeholder-candidates --json` | 0 | 55 candidates，1 uncovered | 5654ms |
| `scan-plugin-source-text --json` | 0 | 94 candidates，扫描 55 文件，忽略 35 文件，high_risk=false | 4024ms |
| `text-scope --json` | 0 | 12577 条，全部 writable | 7110ms |
| `audit-coverage --json` | 1 | `coverage_missing_translation`，pending 12577 | 5470ms |
| `quality-report --json` | 1 | 缺译文、术语、插件/事件/Note 规则、占位符规则 fail-fast | 9063ms |
| `write-back --json` | 1 | `business_error`，规则缺失阻断 | 9206ms |
| `rebuild-active-runtime --json` | 1 | `business_error`，规则缺失阻断 | 9242ms |

副本 `plugins.js` 和 `System.json` 在写入命令前后哈希不变。原始样本 `js/plugins.js` SHA-256 前后不变。临时目录已删除。

## 未验证范围

- 未执行真实模型翻译，避免消耗模型额度。
- 未在规则齐备、译文齐备的真实样本上验证成功写入后的完整审计链。
