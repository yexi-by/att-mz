...
如果 `write-back`、`rebuild-active-runtime` 或手动运行的 `audit-active-runtime` 提示当前运行插件源码有漏翻、坏控制符、未审查候选或来源映射失效，运行 `diagnose-active-runtime` 生成诊断文件。诊断只解释会阻止写入验收的问题，并使用来源映射反推到翻译源审查状态；没有映射或文件哈希不一致时会明确要求重新执行 `rebuild-active-runtime`，不会按当前 JS 文本猜测。修复必须回到规则和已保存译文记录，再重新写进游戏文件。
