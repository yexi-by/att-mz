...
如果 `write-back` 或 `rebuild-active-runtime` 提示当前运行插件源码读取失败或 JS 语法错误，先修复当前运行文件状态后重新写入。手动运行 `audit-active-runtime` 可以完整排查插件源码源文残留和坏控制符；需要反推到已保存译文时，运行 `diagnose-active-runtime` 生成诊断文件。诊断只解释会阻止写入验收的问题，并使用写回映射反推到已保存译文记录；没有映射时会明确报告无法反推，不会按当前 JS 文本猜测。修复必须回到规则和已保存译文记录，再重新写进游戏文件。
